...
static app_led_effect_config_t g_effect_configs[13]; // 13 effects (0-12)
...
static app_led_state_config_t g_state_configs[10] = {
    [APP_LED_OFF] = {0, 0, 0},         [APP_LED_STARTUP] = {0, 0, 0},
    [APP_LED_WIFI_PROV] = {0, 0, 20},  [APP_LED_WIFI_CONN] = {0, 20, 20},
    [APP_LED_OTA] = {10, 0, 10},       [APP_LED_IR_TX] = {20, 0, 0},
    [APP_LED_IR_LEARN] = {20, 20, 0},  [APP_LED_IR_FAIL] = {20, 0, 0},
    [APP_LED_IR_SUCCESS] = {0, 20, 0}, [APP_LED_IDLE] = {0, 20, 0},
};
...
static void led_effect_rainbow(int step, const app_led_effect_config_t *cfg) {
  uint8_t spd = cfg->speed > 0 ? cfg->speed : 50;
  int increment = (spd / 10) + 1;
  for (int i = 0; i < 8; i++) {
...
static void led_effect_running(int step, const app_led_effect_config_t *cfg) {
  if (led_strip)
  ...
  for (int i = 0; i < 3; i++) {
    int p = (pos - i + 8) % 8;
    float dim = 1.0 / (i + 1);
    uint8_t r = cfg->colors[p][0];
    uint8_t g = cfg->colors[p][1];
    uint8_t b = cfg->colors[p][2];
...
static void led_effect_breathing(int step, const app_led_effect_config_t *cfg) {
  float factor = 0.5 * (1 + sin(step * 0.1));
  uint8_t r = cfg->colors[0][0];
  uint8_t g = cfg->colors[0][1];
  uint8_t b = cfg->colors[0][2];
...
static void led_effect_blink(int step, const app_led_effect_config_t *cfg) {
  if ((step / 10) % 2 == 0) {
    uint8_t r = cfg->colors[0][0];
    uint8_t g = cfg->colors[0][1];
    uint8_t b = cfg->colors[0][2];
...
static void led_effect_knight_rider(int step,
  ...
  uint8_t r = cfg->colors[0][0] > 0 ? cfg->colors[0][0] : 255;
  uint8_t g = cfg->colors[0][1];
  uint8_t b = cfg->colors[0][2];
...
static void led_effect_loading(int step, const app_led_effect_config_t *cfg) {
  if (led_strip)
    led_strip_clear(led_strip);
  int pos = step % 8;
  uint8_t r = cfg->colors[0][0];
  uint8_t g = cfg->colors[0][1] > 0 ? cfg->colors[0][1] : 100;
  uint8_t b = cfg->colors[0][2] > 0 ? cfg->colors[0][2] : 255;
...
static void led_effect_sparkle(int step, const app_led_effect_config_t *cfg) {
  uint8_t r = cfg->colors[0][0];
  uint8_t g = cfg->colors[0][1];
  uint8_t b = cfg->colors[0][2];
...
static void run_single_effect(app_led_effect_t effect, int step,
  ...
  default:
    for (int i = 0; i < 8; i++) {
      // cfg is pointer, colors is array
      // cfg->colors[i][0]
...
esp_err_t app_led_init(void) {
  ...
  for (int i = 0; i < 13; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      g_effect_configs[i].colors[j][1] = 20;
...
esp_err_t app_led_save_settings(void) {
  ...
  err = nvs_set_blob(my_handle, "led_configs_v2", g_effect_configs,
...
esp_err_t app_led_load_settings(void) {
  ...
  if (nvs_get_blob(my_handle, "led_configs_v2", g_effect_configs,
